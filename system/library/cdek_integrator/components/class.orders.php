...
class orders extends cdek_integrator implements exchange
{
    ...
    private const DEVELOPER_KEY = 'c6e7304995e8e1f513f1ec380ff89779';
    ...
    private function createPost($order_info = array())
    {
        if (!empty($order_info)) {
            $order = array();
            $order['number'] = !empty($this->number) ? $this->number : $order_info['order_id'];
            ...
            $order['developer_key'] = self::DEVELOPER_KEY;
            ...
            if (!empty($order_info['add_service'])) {
                foreach (array_keys($order_info['add_service']) as $code) {
